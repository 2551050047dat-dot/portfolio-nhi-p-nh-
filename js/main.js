/* ==========================================================================
   ...
   Dùng chung cho toàn bộ 9 trang (nạp trước các script trang riêng).
   ========================================================================== */
...
(function () {
  ...
  TM.fetchJSON = function (relPath) {
    ...
    jsonCache[relPath] = fetch(TM.base + relPath).then(function (res) {
      if (!res.ok) throw new Error("Không tải được " + relPath + " (" + res.status + ")");
      return res.json();
    });
    ...
    window.addEventListener(
      ...
      function () {
        ...
        window.requestAnimationFrame(function () {
          /* Di chuyển nền chậm hơn nội dung → hiệu ứng chiều sâu */
